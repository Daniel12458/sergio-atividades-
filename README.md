# 1. Criando as pastas corretas no projeto
mkdir -p src/main/java/com/seuprojeto/domain
mkdir -p src/main/resources

# 2. Criando o Cliente.java
cat << 'EOF' > src/main/java/com/seuprojeto/domain/Cliente.java
...
# 3. Criando o Pedido.java
cat << 'EOF' > src/main/java/com/seuprojeto/domain/Pedido.java
package com.seuprojeto.domain;
...
# 4. Criando o ItemPedido.java
cat << 'EOF' > src/main/java/com/seuprojeto/domain/ItemPedido.java
package com.seuprojeto.domain;
...
# 5. Criando o Pagamento.java
cat << 'EOF' > src/main/java/com/seuprojeto/domain/Pagamento.java
package com.seuprojeto.domain;
...
# 7. Criando o arquivo de entrega com os seus dados pendentes
cat << 'EOF' > ENTREGA.md
# 1º Trabalho — Implementação das Entidades com JPA
**Nome:** PREENCHA_SEU_NOME_AQUI
**RA:** PREENCHA_SEU_RA_AQUI

Trabalho entregue com sucesso! 
As entidades `Cliente`, `Pedido`, `ItemPedido` e `Pagamento` foram implementadas com Lombok e mapeamento JPA.
O arquivo `import.sql` foi criado com 30 registros mantendo a integridade referencial.
EOF

echo "✅ PRONTO! Todos os arquivos foram criados com sucesso!"
